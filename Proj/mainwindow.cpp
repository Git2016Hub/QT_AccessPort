    ...
    ui(new Ui::MainWindow)
{
    ...
    {
        QSerialPort MainWindow;
        MainWindow.setPort(info);
...
void MainWindow::on_actionOpen_triggered()
{
    ...
    BaudRateType portbaudRate = (BaudRateType)ui->baudRateComboBox->currentText().toInt();   //获取串口名
    DataBitsType portdataBits = (DataBitsType)ui->dataBitsComboBox->currentText().toInt();   //获取串口名
//    QString portName = ui->portNameComboBox->currentText();   //获取串口名
  //  QString portName = ui->portNameComboBox->currentText();   //获取串口名
    ...
    if(myCom->open(QIODevice::ReadWrite)){
        ...
        ui->statusBar->showMessage(tr("Connected to %1 : %2, %3")
                                   .arg(portName).arg(portbaudRate).arg(portdataBits));
                                   //, %4, %5, %6
...
void MainWindow::sendMsg()
{
    ...
    if(ui->sendAsHexcheckBox->isChecked()){
        ...
        buf = ui->textEdit->toPlainText().toAscii();
#else
        buf = ui->textEdit->toPlainText().toLocal8Bit();
#endif
    }
    //发送数据
    myCom->write(buf);
...
/* echo
    ...
    ui->textBrowser->append(tr("[")+ time + tr(" TX]: ") + buf);//newline
    //ui->textBrowser->setText(ui->textBrowser->document()->toPlainText() + buf);//non-breaking
*/
}
...
{
    QString filename = QFileDialog::getOpenFileName(this, tr("读取文件"), 0, tr("保存为 (*.txt *.log *.hex *.bin)"));  //添加更多的文件类型
    QFile file(filename);
    QByteArray all;
    ...
    if(file.size() > MAX_FILE_SIZE){
        QMessageBox::critical(this, tr("载入失败"), tr("文件大小为 %1 字节, 超过限制大小 10000000 字节").arg(file.size()), QMessageBox::Ok);
        return;
    }
    //文件太大时提示是否继续打开
...
//    while(!file.atEnd()){
//        line = file.readLine();
//        ui->textEdit->setText(ui->textEdit->toPlainText() + tr(line));//non-breaking
//    }


    all = file.readAll();
    file.close();
//    ui->textEdit->setText(tr(all));
//    QByteArray voice_fream;

//    for(int num=0;num<file.size();num++)
//    {
//        voice_fream = (tr(0xAAAA) +
//        myCom->write(voice_fream);
//    }
    myCom->write(all);
